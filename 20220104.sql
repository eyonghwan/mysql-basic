...
-- 결과물으 순서대로(오름차순, 내림차순) 정렬하는 기능을 가집니다.
...
-- 정렬시 동점인 부분을 처리하기 위해 OEDER BY 절을 두 개 이상 동시에 나타낼 수 있습니다.
-- 아래 코드는 키로 내림차순을 하되, 동점이면 생년으로 나열합니다.
...
SELECT * FROM userTbl ORDER BY addr DESC, user_name;
...
-- 중복값을 하나로 출력하기 위해 출력컬럼앞에 DISTINCT를 붙입니다.
...
-- 데이커가 많다 보니 모든 데이터를 보여주지않고 
-- 1000개씩만 보여주도록 설정되어있음
...
-- buy 데이터에서 각 인물별로 구매한 개수 총합을 구해보겠습니다.
...
SELECT user_id, sum(price * amount) as tatal FROM buyTbl GROUP BY user_id ORDER BY sum(price * amount);
...
-- 키가 평균보다 지역만 화면에 출력해주세요(userTbl)
...
-- 데이터 정의 언ㅇ로 데이터베이스, 테이블, 뷰, 인덱스 등을 생성하고
...
-- id와 이름쪽에만 데이터를 넣을 때에슨 컬럼명을 생략할 수 없습니다.
INSERT INTO testtbl1(id, nser_name) VALUES (2, '김길동');

-- 만약 컬럼명 지정을 안 했지만 age에 null을 넣고 싶다면
INSERT INTO testtbl1 VALUES (3, '이호영', null);

-- 만약 입력하는 컬럼(열)의 순서를 바꿔서 지정하는 경우는(id, user_name, age가 아닌 순으로 넣음)
-- 모든 컬럼에 값을 입력한는 상황이어도 컬럼명을 따로 기입해야 합니다.
INSERT INTO testtbl1(user_name, age, id) VALUES ('이용환', 10, 4);
