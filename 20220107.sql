...
SELECT CAST(avg(amount) as SIGNED INTEGER) as '평균구매수' FROM buytbl; -- avg(amount)를 INT로 변환

-- CONVET를 써서 자료형을 바꿔 출력해보세요.
...
SELECT '가나가' + 100; -- 숫자로 변환할 수 없는 문자 + 정수 => 정수끼리만 연산
...
-- 문자만 있는 경우 : 0으로 변환
-- 논리식인 경우 0이면 false, 1은 true
SELECT 3 > '2mega';
SELECT 1 > 'AMEGA';

-- <MySQL 내장함수>
-- CONCAT, CAST, CONVERT 등과 같이
-- 내부에 이미 선언되어있어 바로 호출새서 쓸 수 있는
-- 함수들을 보겠습니다.
...
-- 없을때는 ELSE쪽 자료를 출력
-- 단, SQL는 { }로 코드 시작, 끝나는 범위를 표현하지 않기 때문에
...
-- 구분자를 이용해 문자열들을 붙여줍니다.
SELECT CONCAT_WS('--', '1', '3', '4', '5', '10', '22');

-- FORMAT(숫자, 소수점자리)
SELECT FORMAT(1234.588472643356,3);
...
-- 문자열을 길이만큼 늘려놓고, 빈 칸에 채울문자열을 채웁니다.
...
-- 중 하나를 고르면 굅니다.
SELECT TRIM(BOTH 'ㅋ' FROM 'ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㄹㅇㅋㅋㅋㅋㅋㅋㅋㅋ');
SELECT TRIM(LEADING 'ㅋ' FROM 'ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㄹㅇㅋㅋㅋㅋㅋㅋㅋㅋ');
SELECT TRIM(TRAILING 'ㅋ' FROM 'ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㄹㅇㅋㅋㅋㅋㅋㅋㅋㅋ');
...
-- 프로시저 삭제는 DROP PROCEDURE 프로시저명; (프로시저명 뒤에 ()는 뺍니다.)
...
-- 프로시저를 활용해 employee 테이블의 10001번 직원의
...
-- 10001이라는 값을 직접 내부에서 선언하지 않고 프로시저를 호출할 때마다
-- 값을 바꿔넣을 수 있도록 파라미터 처리를 하겠습니다.
DELIMITER $$
